...
import { Activity, AlertCircle, BookOpen, User } from 'lucide-react'
			...
			<div className='grid gap-6'>
					...
					<CardHeader>
						<div className='flex items-center gap-3'>
							...
							<div>
								...
								<CardDescription>
									KPI balli 80 dan past yoki akademik o'zlashtirishi (GPA) 80%
									dan past bo'lgan, grantini yo'qotish arafasidagi talabalar
								</CardDescription>
							</div>
						</div>
					</CardHeader>
				...
				<div className='grid gap-6 lg:grid-cols-2'>
					...
					<Card className='glass-panel'>
						<CardHeader>
							<CardTitle className='text-base sm:text-lg'>
								Reabilitatsiya (Recovery) jarayoni
							</CardTitle>
							<CardDescription>
								Talabaning jarimalarni yuvish va qo'shimcha vazifalarni bajarish
								holati (Max: +10 ball)
							</CardDescription>
						</CardHeader>
						<CardContent className='space-y-4'>
							{rows.map(({ student }) => (
								<div
									key={student.id}
									className='p-4 rounded-lg bg-secondary/10 border border-white/5 space-y-3'
								>
									<div className='flex items-center justify-between'>
										<div>
											<p className='font-semibold text-sm'>{student.name}</p>
											<p className='text-xs text-muted-foreground'>
												{student.group} • Tyutor bahosi: {student.kpi.tutor}/5
												ball
											</p>
										</div>
										<Badge
									...
									{student.recoveryTasks.length > 0 && (
										<div className='grid gap-2 border-t border-white/5 pt-3'>
											{student.recoveryTasks.slice(0, 3).map(task => (
												<div
													key={task.id}
													className='flex flex-col gap-2 rounded-md bg-background/30 p-3 sm:flex-row sm:items-center sm:justify-between'
												>
													<div>
														<p className='text-xs font-medium'>{task.title}</p>
														<p className='text-[11px] text-muted-foreground'>
															{task.deadline} / +{task.recoveryScore} ball /{' '}
															{task.status}
														</p>
													</div>
													<CompleteRecoveryButton
					...
					<Card className='glass-panel'>
						<CardHeader>
							<CardTitle className='text-base sm:text-lg'>
								Mentorlik va Ijtimoiy mas'uliyat
							</CardTitle>
							<CardDescription>
								Nizom bo'yicha talabalarga beriladigan jamoaviy va ijtimoiy
								vazifalar nazorati
							</CardDescription>
						</CardHeader>
						<CardContent className='space-y-4'>
							<div className='p-4 rounded-lg bg-secondary/10 border border-white/5 space-y-2'>
								<div className='flex items-center justify-between'>
									...
									<Badge className='bg-yellow-600/30 text-yellow-300 border-yellow-500/20'>
										3 ball
									</Badge>
								</div>
								<p className='text-xs text-muted-foreground leading-relaxed'>
									O'zlashtirishi past kamida 3 nafar talabaga ustozlik qilish va
									ularning natijalarini yaxshilashda ko'maklashish.
								</p>
							</div>
							...
							<div className='p-4 rounded-lg bg-secondary/10 border border-white/5 space-y-2'>
								<div className='flex items-center justify-between'>
									<span className='font-medium text-sm text-cyan-300 flex items-center gap-2'>
										<Activity className='h-4 w-4' /> Volontyorlik &
										Tashkilotchilik
									</span>
									<Badge className='bg-yellow-600/30 text-yellow-300 border-yellow-500/20'>
										1-2 ball
								...
								<p className='text-xs text-muted-foreground leading-relaxed'>
									...
									ma'naviy tadbirlarni tashkil qilishdagi ishtirok darajasi.
								</p>
							</div>
							...
							<div className='p-4 rounded-lg bg-secondary/10 border border-white/5 space-y-2'>
								...
								<p className='text-xs text-muted-foreground leading-relaxed'>
									PDP Academy, PDP School yoki PDP Junior rahbarlariga ma'muriy,
									tashkiliy hamda strategik yordamchi bo'lib ishlash.
								</p>
							</div>
						</CardContent>
					</Card>
