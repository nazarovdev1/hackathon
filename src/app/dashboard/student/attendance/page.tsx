...
export default async function StudentAttendancePage() {
  ...
  const { attendance_summary, subjects } = attendanceData as {
    attendance_summary: { total_lessons: number; attended: number; absent: number; attendance_percentage: number };
    subjects: SubjectData[];
  };
  const percentage = attendance_summary.attendance_percentage;
  const riskLevel = percentage >= 90 ? "LOW" : percentage >= 75 ? "MEDIUM" : "HIGH";
  ...
  return (
    <DashboardShell title="Davomat" eyebrow="Qatnashuv nazorati">
      <div className="grid gap-6">
        {/* Yuqori statistika */}
        <MotionPanel>
          <div className={`bg-gradient-to-r ${statusBg[riskLevel]} border rounded-lg p-6 relative overflow-hidden`}>
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className={`text-sm font-medium ${statusColors[riskLevel]}`}>
                  {riskLevel === "LOW" ? "Davomat yaxshi" : riskLevel === "MEDIUM" ? "Davomat o'rtacha" : "Davomat past — e'tibor bering!"}
                </p>
                <h2 className="mt-1 text-3xl font-bold tracking-tight">{student.name}</h2>
                <p className="mt-1 text-sm text-muted-foreground">{student.faculty} — {student.group}</p>
              </div>
              <div className="flex items-center gap-4">
                <div className="text-right">
                  <p className="text-xs text-muted-foreground uppercase tracking-wider">Davomat foizi</p>
                  <p className={`text-4xl font-bold ${statusColors[riskLevel]}`}>{percentage}%</p>
                </div>
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{attendance_summary.attended}</p>
                  </div>
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-rose-600 dark:text-rose-400">{attendance_summary.absent}</p>
                  </div>
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold">{attendance_summary.total_lessons}</p>
                  </div>
          ...
          <MotionPanel delay={0.3}>
            <Card className="glass-panel">
              <CardHeader>
                ...
                <CardDescription>Barcha absent holatlar va ularning sabablari</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                {subjects.flatMap((s) =>
                  s.logs
                    ...
                    .map((log, idx: number) => (
                      <div key={`${s.subject_id}-${idx}`} className="rounded-md border border-border/40 bg-secondary/10 p-3">
                        ...
                        {log.reason && (
                          <p className="mt-2 text-xs text-muted-foreground italic">"{log.reason}"</p>
                        )}
                      </div>
                    ))
                )}
                {subjects.flatMap((s) => s.logs.filter((log) => log.status === "absent")).length === 0 && (
                  <div className="text-center py-8 text-muted-foreground">
                    <CheckCircle2 className="h-10 w-10 mx-auto opacity-30 mb-2 text-emerald-500" />
                    <p className="text-sm">Ajoyib! Barcha darslarga qatnashgansiz</p>
                  </div>
                )}
              </CardContent>
            </Card>
