				...
				<div className='grid gap-4 md:grid-cols-2 xl:grid-cols-4'>
					<MotionPanel delay={0.03}>
						<MetricCard
							...
							helper='Asosiy KPI - jarima + tiklanish + ishga joylashish'
							icon={Gauge}
						/>
					</MotionPanel>
					...
					<MotionPanel delay={0.09}>
						<MetricCard
							title='Davomat'
							value={`${student.attendanceTrend.at(-1)?.value}%`}
					...
					<MotionPanel delay={0.12}>
						<MetricCard
							...
							helper='Grant uchun minimal talab - 80%'
							icon={GraduationCap}
						/>
					</MotionPanel>
				</div>
								...
								<TableBody>
									{student.penalties.map(penalty => (
										<TableRow key={`${penalty.date}-${penalty.reason}`}>
											...
											<TableCell className='text-right text-rose-600 dark:text-rose-200'>
												-{penalty.score}
											</TableCell>
										</TableRow>
									))}
						...
						<CardContent className='flex-1 flex flex-col justify-between'>
							<div>
								...
								<p className='mt-3 text-sm text-muted-foreground'>
									{student.recoveryStatus}% qayta tiklash rejasi bajarildi
								</p>
							</div>
							<div className='mt-5 grid gap-3'>
								{recommendations.map(item => (
									<div
										key={item.title}
										className='rounded-md border border-border/40 bg-secondary/20 p-3'
									>
				...
				<div className='grid gap-5 lg:grid-cols-2'>
					...
					<Card className='glass-panel'>
						<CardHeader className='pb-4'>
							<CardTitle className='text-base sm:text-lg'>
								Asosiy reyting (KPI) ballari taqsimoti
							</CardTitle>
						</CardHeader>
						<CardContent className='space-y-4'>
							{[
								...
								return (
									<div
										key={item.label}
										className='p-3.5 rounded-lg bg-secondary/20 border border-border/40 space-y-3 transition-all duration-200 hover:border-border/60 hover:bg-secondary/35'
									>
										...
										<div className='h-2 w-full rounded-full bg-secondary/60 overflow-hidden'>
											<div
												className={`h-full rounded-full bg-linear-to-r ${item.color} transition-all duration-500`}
												style={{
													width: `${percent}%`,
					...
					<div className='flex flex-col gap-4'>
						<h3 className='text-base sm:text-lg font-semibold px-1'>
							Talabaning yutuqlari
						</h3>
						<div className='grid gap-4 sm:grid-cols-2 lg:grid-cols-1'>
							{student.achievements.map(achievement => (
								<Card
									key={achievement.title}
									className='glass-panel transition-all duration-300 hover:translate-x-1 hover:border-cyan-500/30'
								>
									<CardContent className='flex items-center gap-4 p-5'>
										...
										<div>
											...
											<p className='text-xs text-muted-foreground mt-1'>
												{achievement.category} / +{achievement.score} ball
											</p>
										</div>
									</CardContent>
								</Card>
