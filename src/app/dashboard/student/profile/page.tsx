...
export default async function StudentProfilePage() {
	...
	const mentor = await prisma.user.findFirst({
		where: { mentorStudents: { some: { id: student.id } } },
		select: {
			fullName: true,
			email: true,
			mentorProfile: { select: { specialty: true } },
		},
	})
	...
	const tutor = await prisma.user.findFirst({
		...
		select: {
			fullName: true,
			email: true,
			tutorProfile: { select: { assignedGroup: true } },
		},
	})

	const attendancePercentage =
		attendanceData?.attendance_summary.attendance_percentage ?? 0
	const totalAchievements = student.achievements.filter(
		a => a.status === 'APPROVED',
	).length
	const totalScore = student.achievements
	...
	return (
		<DashboardShell title='Profil' eyebrow="Shaxsiy ma'lumotlar">
			<div className='grid gap-6'>
				{/* Profil header */}
				<MotionPanel>
					<div className='bg-linear-to-r from-primary/10 via-primary/5 to-transparent border border-primary/20 rounded-lg p-6 relative overflow-hidden'>
								...
								{mentor && (
									<>
										<div className='border-t border-border/40 pt-3'>
											...
											<p className='text-xs text-muted-foreground'>
												{mentor.mentorProfile?.specialty}
											</p>
										</div>
									</>
								)}
								{tutor && (
									<>
										<div className='border-t border-border/40 pt-3'>
											...
											<p className='text-xs text-muted-foreground'>
												Guruh: {tutor.tutorProfile?.assignedGroup}
											</p>
										</div>
									</>
								)}
				...
				<MotionPanel delay={0.2}>
					<Card className='glass-panel'>
						<CardHeader>
							...
							<CardDescription>
								Har bir yo'nalish bo'yicha olingan ballar
							</CardDescription>
						</CardHeader>
						<CardContent>
