...
export default async function StudentRatingPage() {
	...
	const currentPosition =
		leaderboard.find(item => item.id === currentStudent.id)?.position ?? 0
	...
	return (
		<DashboardShell title='Reyting jadvali' eyebrow='KPI reytingi'>
			<div className='grid gap-6'>
				...
				<MotionPanel>
					<div className='bg-linear-to-r from-emerald-500/10 via-teal-500/5 to-transparent border border-emerald-500/20 rounded-lg p-6 relative overflow-hidden'>
							...
							<div className='flex items-center gap-3'>
								...
								<div>
									...
									<p className='text-lg font-bold text-foreground'>
										{leaderboard.find(item => item.id === currentStudent.id)
											?.score ?? 0}{' '}
										ball
									</p>
								</div>
					...
					{leaderboard.slice(0, 3).map((item, idx) => {
						...
						return (
							<MotionPanel delay={(idx + 1) * 0.05} key={item.id}>
								<Card
									className={`glass-panel border-l-4 border-l-${idx === 0 ? 'amber-500' : idx === 1 ? 'slate-400' : 'amber-700'}`}
								>
										...
										{leaderboard.map(item => {
											...
											return (
												<TableRow
													...
													className={
														isSelf
															? 'bg-emerald-500/5 hover:bg-emerald-500/10 font-medium transition-colors'
															: ''
													}
												>
