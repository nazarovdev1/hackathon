...
export function AttendanceCalendar({ subjects }: { subjects: SubjectData[] }) {
	...
	const allDates = Array.from(new Set(filteredLogs.map(log => log.date)))
				...
				<div className='grid grid-cols-7 gap-1.5'>
					...
					{allDates.map(date => {
						...
						const hasPresent = dayLogs.some(
							log => log.status === 'attended' || log.status === 'late',
						)
						...
						return (
							...
							>
								...
								{hasAbsent && (
									<XCircle className='h-3 w-3 text-rose-500 mt-0.5' />
								)}
							</div>
						)
					})}
