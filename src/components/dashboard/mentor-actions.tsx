...
import { CheckCircle2, Loader2, MessageSquare, Plus, TriangleAlert } from "lucide-react";
import {
  assignRecoveryTask,
...
export function MentorStudentActions({ student }: { student: StudentDashboardSnapshot }) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [status, setStatus] = useState<ActionStatus | null>(null);
  const [dialog, setDialog] = useState<"feedback" | "recovery" | "penalty" | null>(null);
  const [feedbackSubject, setFeedbackSubject] = useState("Grant bo'yicha tavsiya");
  const [feedbackMessage, setFeedbackMessage] = useState(buildDefaultFeedback(student));
  const [recoveryTitle, setRecoveryTitle] = useState("Recovery vazifasi");
  const [recoveryDescription, setRecoveryDescription] = useState(buildDefaultRecovery(student));
  ...
  const penaltyLimit = Math.max(0, Number((20 - student.kpi.penalty).toFixed(2)));
  ...
  function submitRecovery() {
    if (recoveryLimit <= 0 || recoveryScore <= 0 || recoveryScore > recoveryLimit) return;
    runAction(
      () =>
        assignRecoveryTask({
          studentId: student.id,
  ...
  function submitPenalty() {
    if (penaltyLimit <= 0 || penaltyScore <= 0 || penaltyScore > penaltyLimit) return;
    runAction(
      () =>
        createPenalty({
          studentId: student.id,
    ...
    <div className="flex flex-col items-end gap-2">
      <div className="flex flex-wrap justify-end gap-2">
        ...
        <Button
          ...
          title={recoveryLimit <= 0 ? "Recovery limiti tugagan yoki jarima yo'q" : undefined}
        >
        ...
        <Button
          ...
          title={penaltyLimit <= 0 ? "Jarima limiti -20 ballga yetgan" : undefined}
        >
          <TriangleAlert className="mr-1 h-3.5 w-3.5" /> Jarima
        </Button>
      </div>
      {status && (
        <p className={status.kind === "success" ? "text-xs text-emerald-400" : "text-xs text-rose-400"}>
          {status.message}
        </p>
      )}

      <Dialog open={dialog === "feedback"} onOpenChange={(open) => setDialog(open ? "feedback" : null)}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>Tavsiya yuborish</DialogTitle>
            <DialogDescription>{student.name} profiliga mentor/tutor tavsiyasi yoziladi.</DialogDescription>
          </DialogHeader>
          <form className="grid gap-4" onSubmit={(event) => event.preventDefault()}>
            <Field label="Mavzu">
              <Input value={feedbackSubject} onChange={(event) => setFeedbackSubject(event.target.value)} required />
            </Field>
            <TextareaField label="Tavsiya matni" value={feedbackMessage} onChange={setFeedbackMessage} />
            <DialogFooter>
              <Button type="button" disabled={isPending || !feedbackMessage.trim()} onClick={submitFeedback}>
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <form className="grid gap-4" onSubmit={(event) => event.preventDefault()}>
            <Field label="Vazifa nomi">
              <Input value={recoveryTitle} onChange={(event) => setRecoveryTitle(event.target.value)} required />
            </Field>
            <TextareaField label="Vazifa tavsifi" value={recoveryDescription} onChange={setRecoveryDescription} />
            <div className="grid gap-3 sm:grid-cols-2">
              <Field label="Recovery ball">
                <Input
                  ...
                  step="0.5"
                  value={recoveryScore}
                  onChange={(event) => setRecoveryScore(Number(event.target.value))}
                  required
                />
              </Field>
            ...
            <DialogFooter>
              <Button type="button" disabled={isPending || recoveryLimit <= 0 || recoveryScore <= 0 || recoveryScore > recoveryLimit} onClick={submitRecovery}>
      ...
      <Dialog open={dialog === "penalty"} onOpenChange={(open) => setDialog(open ? "penalty" : null)}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Semestr bo'yicha qolgan jarima limiti: {penaltyLimit} ball.</DialogDescription>
          </DialogHeader>
          <form className="grid gap-4" onSubmit={(event) => event.preventDefault()}>
            ...
            <DialogFooter>
              <Button type="button" disabled={isPending || penaltyLimit <= 0 || penaltyScore <= 0 || penaltyScore > penaltyLimit} onClick={submitPenalty}>
...
export function CompleteRecoveryButton({ taskId, disabled }: { taskId: string; disabled?: boolean }) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
