...
type StudentWithDashboardData = Prisma.StudentProfileGetPayload<{
...
function buildAttendanceTrend(
	records: StudentWithDashboardData['attendanceRecords'],
) {
	const grouped = new Map<
		string,
		{ date: Date; earned: number; total: number }
	>()
	...
	for (const record of records) {
		...
		const earned =
			record.status === 'ABSENT' ? 0 : record.status === 'LATE' ? 0.5 : 1
		grouped.set(key, {
			date: current.date,
...
function buildRecoveryStatus(tasks: StudentWithDashboardData['recoveryTasks']) {
	...
	return Math.round(
		statusValues.reduce((sum, value) => sum + value, 0) / statusValues.length,
	)
}

function buildStudentSnapshot(
	student: StudentWithDashboardData,
): StudentDashboardSnapshot {
...
export async function getCurrentStudentFeedback() {
	...
	return student.feedbacks.map(item => ({
		...
		role:
			item.author.mentorProfile?.specialty ??
			(item.author.tutorProfile ? 'Tyutor' : item.author.role),
...
export async function createPenalty(input: {
	...
	const score = normalizeScore(
		...
		1,
		20,
		"Jarima balli 1 va 20 oralig'ida bo'lishi kerak.",
	)
	const currentPenalty = await getLatestScoreValue(
		input.studentId,
		'penaltyScore',
	)
	...
	const penalty = await prisma.$transaction(async tx => {
		...
		await bumpLatestScore(tx, input.studentId, {
			penaltyScore: currentPenalty + score,
		})
		return created
	})
...
export async function assignRecoveryTask(input: {
	...
	const penaltyScore = await getLatestScoreValue(
		input.studentId,
		'penaltyScore',
	)
	const currentRecovery = await getLatestScoreValue(
		input.studentId,
		'recoveryScore',
	)
	...
	if (currentRecovery + recoveryScore > allowedRecovery) {
		throw new Error(
			`Recovery jami ${allowedRecovery} balldan oshmasligi kerak (jarimaning 50%).`,
		)
	}
	...
	const task = await prisma.$transaction(async tx => {
		...
		await bumpLatestScore(tx, input.studentId, {
			recoveryScore: currentRecovery + recoveryScore,
		})
		return created
	})
...
export async function uploadAchievement(input: {
	...
	const studentId =
		user.role === 'STUDENT' ? user.studentProfileId : input.studentId
...
export async function updateAchievement(input: {
	...
	if (user.role === 'STUDENT') {
		await assertStudentAccess(user, achievement.studentId)
		if (achievement.status !== 'PENDING')
			throw new Error("Approved/rejected achievements can't be edited")
	}
...
export async function deleteAchievement(input: { achievementId: string }) {
	...
	if (user.role === 'STUDENT') {
		await assertStudentAccess(user, achievement.studentId)
		if (achievement.status !== 'PENDING')
			throw new Error("Approved/rejected achievements can't be deleted")
	}

	await prisma.achievement.delete({ where: { id: input.achievementId } })
	return { id: input.achievementId }
}
...
async function assertStudentAccess(
	...
	if (!student)
		throw new Error("Bu talaba bo'yicha amal bajarishga ruxsat yo'q.")
}

async function getLatestScoreValue(
	studentId: string,
	field: MutableScoreField,
) {
