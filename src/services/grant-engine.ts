...
export function calculateGrantScore(
	input: GrantKpiInput,
): GrantCalculationResult {
	...
	const finalScore = clampScore(
		mainKpi -
			input.penalty +
			input.recovery +
			input.employmentBonus +
			input.adminBonus,
	)
	const academicDenied = input.academicPercent < 80
	const grantStatus = academicDenied
		? 'DENIED'
		: finalScore >= 80
			? 'ELIGIBLE'
			: 'RISK'
	...
	if (academicDenied)
		reasons.push('Academic percent 80% dan past, grant avtomatik rad etiladi.')
	if (finalScore < 80)
		reasons.push('Final score grant eligibility chegarasidan past.')
	if (input.penalty > 0)
		reasons.push('Penalty ballari final natijani kamaytiryapti.')
	if (input.adminBonus > 0)
		reasons.push("Admin tomonidan bonus ball qo'shilgan.")
	if (input.attendance < 12)
		reasons.push('Attendance komponenti monitoring talab qiladi.')
